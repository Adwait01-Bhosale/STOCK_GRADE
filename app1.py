...
if comp=="Kotak":
    ...
    kotak = open("kotak_predictor.pkl", "rb")
    model = pickle.load(kotak)
    data = pd.read_csv("C:/Users/User/Desktop/StockGrade/StockGrade/KOTAKBANK.csv")
    ...
    if nav == "Home":
        ...
        if st.button("Not understanding where to start?"):
            st.graphviz_chart("""
            digraph{
            HomePage->UnderstandTheInterface->
            CheckSidebar->UnderstandTheGraphs->
            GoToFAQ->GetClarifiedWithYourDoubts->
            GoToCompany->ChooseYourFavouriteOne->
            TryMakingSomePredictions->ByGoingToPredictor
            GetExperiencedWithStockGrade->EarnHard
            }""")
        ...
        if st.button("Want To know the Growth History Column Wise?"):
            ...
            ax3.plot(data['DeliverableVolume'],data['Close'])
            ax4.scatter(data['Deliverble'],data['Close'])
            st.pyplot(fig)
    ...
    if nav == "Predictor":
        ...
        PrevClose = st.number_input("Enter the Previous Close Price", 27.00, 2040.00, step=40.0)
        Low = st.number_input("Enter the Low Price", 27.00, 2000.00, step=40.0)
        VWAP = st.number_input("Enter VWAP", 27.00, 2038.00, step=40.0)
        Volume = st.number_input("Enter the Volume", 40.00, 3000.00, step=200.0)
        DeliverableVolume = st.number_input("Enter Deliverable Volume", 40.0, 3000.0, step=200.0)
        Deliverble = st.number_input("Enter the Deliverble Value", 0.001, 100.0, step=10.0)
    ...
    if nav == "Graphology":
        ...
        if graph == "Previous Close Price Vs Close Price":
            ...
            prevclo = sns.relplot(data=data, x='PrevClose', y='Close')
            st.pyplot(prevclo)
